...
export default function App() {
  ...
  // S21 Mode State
  const [useS21Mode, setUseS21Mode] = useState(false);
  
  // Session Zoom State (Persists only during session, resets on reload)
  const [sessionZoom, setSessionZoom] = useState<number | null>(null);
  ...
  const handleS21ModeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      setUseS21Mode(e.target.checked);
      // Reset session zoom when mode switches so the new mode's default applies
      setSessionZoom(null);
  };
    ...
    return (
      <div className="h-[100dvh] w-full flex flex-col items-center justify-center bg-slate-900 text-white p-6 relative">
        ...
        <div className="flex flex-col gap-6 items-center w-full max-w-xs">
            ...
            {/* S21 Mode Toggle */}
            <label className="flex items-center gap-3 bg-slate-800/50 px-4 py-3 rounded-lg border border-slate-700 cursor-pointer hover:bg-slate-800 transition-colors w-full justify-center">
                <div className="relative flex items-center">
                    <input 
                        type="checkbox" 
                        checked={useS21Mode}
                        onChange={handleS21ModeChange}
                        className="w-5 h-5 accent-emerald-500 rounded focus:ring-emerald-500" 
                    />
                </div>
                <div className="flex flex-col text-left">
                    <span className="text-sm font-bold text-slate-200">S21 / 근접 초점 보정 모드</span>
                    <span className="text-[10px] text-slate-400">초점이 잘 안 맞을 때 체크하세요</span>
                </div>
            </label>
        ...
        {view === 'scan' ? (
          <>
            <Scanner 
                ...
                onError={handleError} 
                isPaused={isPaused} 
                s21Mode={useS21Mode} // Pass the mode
                initialZoom={sessionZoom} // Pass session zoom
                onZoomChange={setSessionZoom} // Pass zoom setter
            />
